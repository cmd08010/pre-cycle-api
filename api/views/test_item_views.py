...
from ..serializers import ItemSerializer, UserSerializer, ItemGetSerializer, TestItemSerializer, TestItemGetSerializer

# Create your views here.


class TestItems(generics.ListCreateAPIView):
    """For my test admin users so they can test the admin features"""
    ...
    def get(self, request):
        ...
        test_items = TestItem.objects.all()

        # Run the data through the serializer
        data = ItemGetSerializer(items, many=True).data
        test_data = TestItemGetSerializer(test_items, many=True).data
        print(test_items, "the data")
        return Response({'items': data, 'test_items': test_data})
    ...
    def post(self, request):
        ...
        item = TestItemSerializer(data=request.data)
...
class TestItemsGetDetail(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = (IsAuthenticated,)
    ...
    def get(self, request, pk):
        ...
        data = TestItemGetSerializer(item).data
        return Response({'items': [data]}, status=status.HTTP_201_CREATED)


class TestItemsDetail(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = (IsAuthenticated,)
    ...
    def get(self, request, slug):
        ...
        item = TestItem.objects.filter(barcode=slug)
        ...
        if not item:
            ...
            item = get_object_or_404(TestItem, name=slug)
            data = TestItemGetSerializer(item).data
            print(data, "my data")
            return Response({'items': [data]}, status=status.HTTP_201_CREATED)
    ...
    def partial_update(self, request):
        ...
        item = get_object_or_404(TestItem, pk=p_k)
        ...
        data = TestItemSerializer(item, data=request.data, partial=True)
